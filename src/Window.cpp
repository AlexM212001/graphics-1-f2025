...
void CreateWindow(int width, int height, const char* title)
{
    ...
    // initialize mouse previous position so first delta is small/zero
    double mx, my;
    glfwGetCursorPos(g_app.window, &mx, &my);
    g_app.mouse_prev_x = mx;
    g_app.mouse_prev_y = my;
    g_app.mouse_initialized = true;
}
...
void BeginFrame()
{
    ...
    // compute mouse delta here so main() can call GetMouseDelta() after BeginFrame()
    double mx, my;
    glfwGetCursorPos(g_app.window, &mx, &my);

    if (!g_app.mouse_initialized)
    {
        g_app.mouse_prev_x = mx;
        g_app.mouse_prev_y = my;
        g_app.mouse_initialized = true;
        g_app.mouse_dx = g_app.mouse_dy = 0.0f;
    }
    else
    {
        double dx = mx - g_app.mouse_prev_x;
        double dy = my - g_app.mouse_prev_y;

        // If ImGui wants the mouse, do not forward movement to the app/camera
        ...
        {
            // flip vertical so moving mouse up yields positive y for camera pitch
            g_app.mouse_dx = (float)dx;
            g_app.mouse_dy = (float)(-dy);
        }
...
Vector2 GetMouseDelta()
{
    ...
    // consume the delta so subsequent calls this frame return zero
    g_app.mouse_dx = 0.0f;
    g_app.mouse_dy = 0.0f;
    return v;
}
